WITH ndt_filtered AS (
  SELECT
  *,
  CASE WHEN client.Network.ASNumber = 14593 THEN "sl" ELSE "other" END Operator,
  FROM 
    ndt.ndt7
  WHERE 
...
data AS (
  ...
  *,
  ...
  FROM ndt_filtered
  WHERE ARRAY_LENGTH(raw.Upload.ServerMeasurements) >= 1
  AND (
    SELECT
...
SELECT
  ...
  APPROX_QUANTILES(m_upload.diffRTT, 100) AS up_queuedelay_percentiles,
